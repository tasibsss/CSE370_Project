...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    if (isset($_POST['delete_flight'])) {
        ...
        // Check if both Flight_number and ticket_id are provided
        if (!empty($Flight_number) && !empty($ticket_id)) {
            // Check if the provided Flight_number and ticket_id match a booking
            $check_booking_query = "SELECT ticket_type FROM Booking WHERE username = '{$_SESSION['username']}' 
                                    AND Flight_number = '$Flight_number' AND ticket_id = '$ticket_id'";
            ...
            if (mysqli_num_rows($result) == 1) {
                ...
                // Delete the booking
                $delete_booking_query = "DELETE FROM Booking WHERE username = '{$_SESSION['username']}' 
                                         AND Flight_number = '$Flight_number' AND ticket_id = '$ticket_id'";
                ...
                if (mysqli_query($conn, $delete_booking_query)) {
                    ...
                    if ($ticket_type === 'economy') {
                        mysqli_query($conn, "UPDATE Flight SET eco_seats = eco_seats + 1 WHERE Flight_number = '$Flight_number'");
                    } elseif ($ticket_type === 'business') {
                        mysqli_query($conn, "UPDATE Flight SET bus_seats = bus_seats + 1 WHERE Flight_number = '$Flight_number'");
                    }
                } else {
